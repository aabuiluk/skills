...
**Гейт публікації:** студентам видно тільки пари, де власник сам поставив
галочку. `public_plan.json` **не чіпати** і **не проставляти галочки кодом**.
Галочка в `/courses/python_ai_materials` відкриває ту саму пару студентам на
`/course/python_ai_step` (`student_visible_pairs`) — **без** проставляння
галочок на step. Деплой не-`presentation` файлів — лише з `--force`.
Див. [[step-publish-gate]].

**Materials (окремий курс):** густина лекції — еталон пар **11–16**
([gold-11-16.md](../teacher-materials-skill/gold-11-16.md)), не генератор 15–50.
Скіл: `teacher-materials-skill`. **Заборонено** супер-скорочені material-версії
(інцидент: v2 на парах 15–16 ховала розгорнутий шар). Нова версія ≥ попередньої
повноти. Завдання — з прикладом виконання для викладача.
...
**Material v2.0+ (еталони густини):**

| Версія | Пари | Що це |
|--------|------|--------|
| 2.0 / 3.0 | 9–14 | простий шлях + (3.0) графіки в коді на 13–14 |
| 4.0 | +15–16 | **розгорнуті** teacher notes + extra (не тонкий скелет) |
| 5.0 | +17–18 | той самий рівень щільності, що v4 15–16 |

Еталон голосу/темпу простого шляху — global_index **9 і 10**, плюс повні пари
**11–16** як планка інформації. Admin: два **випадаючі** селектори (не pills)
«Студенти» і «Версія». Таймінг: `data-timing="required|optional"`.

**АНТИ-ПАТЕРН (не повторювати):** коротка v2 на 15–16 через
`until="3.0"` з урізаними нотатками при наявності повного `from="4.0"`.
Нова версія **не має** зменшувати текст для охоплених пар.
...
4. **Завдання** `id="itstep-task-N"` · `<span class="kind task">практика ITSTEP</span>` · таймер. Всередині: `.callout tip` з `data-task-level` (бейдж рівня) → **`.callout tip` «Звідки дані і навіщо»** (або «Що рахуємо…») — перед кодом: що за датасет, що означає `y`, чому ця колонка/рядок, що дасть результат (див. §11, п. 5) → `.callout example` / офіційне завдання **і прямим лінком на LMS** (див. §6) + орієнтир → **`.lh-teacher-note` з прикладом виконання** (кроки, навмисна помилка, очікуваний вивід, час) → `.callout your-task` / X.1 без готового файлу. У розгорнутих версіях (v4+) додатково `teacher-deep-dive` «завдання по хвилинах». У v2-парах розбір першого-двох завдань можна показувати розгорнутим: `<details open class="callout solution">` (еталон пар 9–10 v4) — але **нотатка викладача з walkthrough обовʼязкова завжди**.
...
## 12. Material versions + простий шлях + таймінг

**Еталон густини:** пари **9–10** (голос) + **11–16** (планк інформації) +
**v4 на 15–16** / **v5 на 17–18** (розгорнуті teacher notes). Перед новою
версією — читати HTML еталонів цілком.

**АНТИ-ПАТЕРН (не повторювати):** супер-скорочена v2 на 15–16
(`until="3.0"` з урізаними нотатками при повному `from="4.0"`). Нова версія
для пари в `overrides` **не тонша** за попередній повний шар. Не публікувати скелет.

Також v2-шар мають пари **11–14**. `overrides.*.pairs` = **global_index**.

**Коли:** додавати повний простий шлях одразу (lead + приклади + timers), не скелет.
...
| `data-material-version="1.0"` … `"5.0"` | шари |
| `data-material-version-from="5.0"` | від цієї версії й далі |
| без мітки | спільні блоки — тримати **повний** контент |

Правила:
- Teacher walkthrough на завданнях — спільний або `from` поточної версії **тієї ж** щільності; не класти урізаний дубль у `until`.
- Admin toolbar: два **випадаючі** меню (не pills) — «Студенти» + «Версія».
- Lead: обгортки `<div data-material-version-…>` навколо `<p class="lead">`.
- Публічний URL без `?material_version=`.
...
пізніше». **Одразу** — повний маршрут як у парах 9–10 / 11–16.
