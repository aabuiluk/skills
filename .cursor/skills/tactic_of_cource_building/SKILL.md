...
Схема **одна** на всі пари курсу. Не вигадуй пʼяту зону. Не зливай `task` і `example`. Не став Checkpoint одразу після першого завдання. Не зливай два положення `theory` в один `text`.
...
Чотири зони завжди. `toc` один у `rail`. `banner` перший у `stage`. `exit` перед `deep-study`. Teacher-близнюк після кластера, який веде. Кожен `theory` — два положення `коротко` / `детально` в тому ж `id`.

**Готово:** повний список слотів, як у схемі.

### 5. Fill

Є факт/умова/лінк — впиши в `text` одне-два речення або цитату джерела. Немає — `Потребує допрацювання`. Слот не видаляй. Для `theory` заповнюй **обидва** `positions`; дірка в одному положенні не виправдовує порожнє друге.

Голос студентських типів (`theory`, `example`, `callout`, `trap`, `task`, …) — до читача. `teacher` може говорити «покажіть».

**Готово:** жоден слот без `text`; кожен `theory` має `positions.коротко` і `positions.детально`; дірки позначені явно.
...
Поруч короткий `pair-NN-skeleton.md`: таблиця `type | title | хв | статус`. Для `theory` два рядки статусу: `коротко` / `детально` (заповнено / `Потребує допрацювання`).
...
Не: `theory` без одного з положень `коротко` / `детально`, або сторінковий `extra «Коротко / детально»` замість положень.  
...
- Кожен `theory` має обидва `positions`; порожнє положення = `Потребує допрацювання`.
