...
Еталон густини — пари **11–16** ([gold-11-16.md](gold-11-16.md); вузько 15–16 — [gold-15-16.md](gold-15-16.md)): багато `lead` і «Приклад ·» **до** абстракції, не одна картка на тему. Super-короткі material-версії заборонені.

1. Banner `#agenda`: kicker, h2, lead, `.timing-strip` (сума pill = 80 хв), `.goals`, `.term-hint`.
2. Теорія `article.block` + `kind main`: `data-minutes` і `.clock` зі слотом. Чергувати з окремими блоками «Приклад ·».
3. **Extra з KB** після останньої теорії, **перед** практикою: `kind deep`, без HH:MM пари. Типовий id: `extra-kb-…`.
4. Практика `kind task` (ITSTEP або курована) + «Звідки дані і навіщо».
5. **Checkpoint** (3 хв) і **exit ticket** (4 хв), `kind_label` «перевірка», входять у 80.
6. **`deep-study`** в кінці: take-home ~30 хв, без слоту вікна.
7. Усередині extra дозволений другий шар `<div class="extra-more" data-kb="extra-more">` — не чіпай timed-годинники.
...
- **Обовʼязково:** `.lh-teacher-note` з **прикладом виконання** (кроки live, навмисна помилка, очікуваний вивід / таблиця, час).
- X.1: «зробіть за аналогією» + `.callout.task` «Ваше рішення». Без коду розвʼязку студенту.

Повні розвʼязки X для роздатку — у `lesson_materials/…/results/` після білдера; на вебі викладач має walkthrough у нотатці.
